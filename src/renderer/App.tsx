...
const App: React.FC = () => {
  ...
  const loadDirectory = useCallback(
    async (dirPath: string) => {
      if (!dirPath) return;
      const isTrash = dirPath === TRASH_PATH;
      if (isTrash) setFiles([]);
      setIsLoading(true);
      ...
      try {
        // @ts-ignore
        const loadedFiles = isTrash
          // @ts-ignore
          ? await window.electronAPI.listTrash()
          // @ts-ignore
          : await window.electronAPI.readDirectory(dirPath, { showHidden });
  ...
  const handlePaste = useCallback(async () => {
    ...
    if (failed > 0) {
      toast(
        `Failed to ${data.type === "copy" ? "copy" : "move"} ${failed} item(s)`,
        "error",
      );
    }
    ...
    async (file: any) => {
      if (file.isDirectory) {
        ...
        try {
          ...
          useExplorerStore
            .getState()
            .addToast(e.message || "Cannot open file", "error");
        }
      }
    },
    [setCurrentPath],
  ...
  return (
    <div className="flex flex-col h-screen bg-white text-gray-800">
      ...
      <div className="flex items-center gap-2 p-2 border-b bg-gray-50">
        ...
        <input
          type="text"
          value={currentPath === TRASH_PATH ? "Trash" : currentPath}
          readOnly
          className="flex-1 px-3 py-1 border rounded bg-white mx-2"
        />
        ...
        {viewMode !== "column" && (
          <input
            ...
            className="w-48 px-3 py-1 border rounded bg-white mr-2"
          />
        )}
        ...
        <div className="flex bg-white border rounded">
          <button
            ...
            className={`p-1 ${viewMode === "details" ? "bg-blue-100" : "hover:bg-gray-100"}`}
            title="Details View"
          >
            <List size={20} />
          </button>
          ...
          <button
            ...
            className={`p-1 ${viewMode === "column" ? "bg-blue-100" : "hover:bg-gray-100"}`}
            title="Column View"
          >
            <Columns size={20} />
          </button>
        ...
        {viewMode === "grid" && (
          <div className="flex bg-white border rounded ml-2">
            <button
              ...
              className={`p-1 text-xs ${gridSize === "small" ? "bg-blue-100" : "hover:bg-gray-100"}`}
            >
              S
            </button>
            <button
              onClick={() => setGridSize("medium")}
              className={`p-1 text-sm ${gridSize === "medium" ? "bg-blue-100" : "hover:bg-gray-100"}`}
            >
              M
            </button>
            <button
              onClick={() => setGridSize("large")}
              className={`p-1 text-base ${gridSize === "large" ? "bg-blue-100" : "hover:bg-gray-100"}`}
            >
              L
            </button>
          </div>
